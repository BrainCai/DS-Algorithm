...
/**
 * 给定一个数组，该数组为[1,n]连续整数组，求出其所有子集 递归算法。
 * 
...
public class GetAllChildSubsetSolInRecur {
	...
	public List<List<Integer>> getAllChildListSet(int idx, int[] nums) {
		...
		if (idx >= nums.length) {
			allChildListSet = new ArrayList<>();// 到达边界条件后创建所有子集集合
			allChildListSet.add(new ArrayList<>());// 当数组遍历结束后，恰好是空集，也是其子集
		} else {
			allChildListSet = getAllChildListSet(idx + 1, nums);
			// 取出当前的元素
			int item = nums[idx];
			// 创建临时子集集合，将当前元素分发到当前allChildListSet中的子集中，并将其append到allChildListSet中
			List<List<Integer>> tmpChildListSet = new ArrayList<>();
			// 遍历allChildListSet中的所有子集，将当前元素添加到所有子集中
			...
			// 将tmpChildListSet追加到(append)allChildListSet中
